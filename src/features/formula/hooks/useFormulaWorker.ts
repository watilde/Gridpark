...
export function useFormulaWorker(tabId: string) {
  ...
  useEffect(() => {
    ...
    worker.onmessage = (event: MessageEvent<WorkerResponse>) => {
      ...
      if (message.type === 'RESULT') {
        ...
        console.log('[useFormulaWorker] Result:', {
          cellRef,
          result,
        ...
        console.log('[useFormulaWorker] Batch result:', {
        ...
        setStats(prev => ({
          ...
          totalDuration: prev.totalDuration + duration,
          averageDuration:
            (prev.totalDuration + duration) / (prev.totalCalculations + results.length),
        }));
  ...
  const calculate = useCallback(
    (formula: string, cellRef: string): Promise<number | string> => {
  ...
  const calculateBatch = useCallback(
    (
      formulas: Array<{ cellRef: string; formula: string }>
    ): Promise<Array<{ cellRef: string; result: number | string }>> => {
      return new Promise((resolve, reject) => {
        if (!workerRef.current || !isReady) {
          reject(new Error('Worker not ready'));
          return;
  ...
  const getDependencies = useCallback(
    (cellRef: string): Promise<{ dependencies: string[]; dependents: string[] }> => {
      return new Promise((resolve, reject) => {
        if (!workerRef.current || !isReady) {
          reject(new Error('Worker not ready'));
          return;
  ...
  const invalidateCache = useCallback(
    (row: number, col: number): Promise<void> => {
      return new Promise((resolve, reject) => {
        if (!workerRef.current || !isReady) {
          reject(new Error('Worker not ready'));
          return;
  ...
  const checkCircularReferences = useCallback((): Promise<{
    hasCircularRefs: boolean;
    circularCells: string[];
  }> => {
    return new Promise((resolve, reject) => {
