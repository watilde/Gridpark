...
export const SpreadsheetGrid: React.FC<SpreadsheetGridProps> = ({
  ...
  const handleScroll = useCallback(() => {
    ...
    const endRow = Math.min(
      rows,
      Math.ceil((container.scrollTop + container.clientHeight) / rowHeight) + overscan
    );
    const startCol = Math.max(0, Math.floor(container.scrollLeft / colWidth) - overscan);
    const endCol = Math.min(
      columns,
      Math.ceil((container.scrollLeft + container.clientWidth) / colWidth) + overscan
    );
    ...
    <GridContainer>
      <GridScrollContainer ref={scrollContainerRef}>
        ...
        >
          ...
          {Array.from({ length: columns }, (_, col) => (
            <HeaderCell key={`header-${col}`}>{getColumnName(col)}</HeaderCell>
          ))}
      ...
      >
        <Typography level="body-xs">
          Selected: {getColumnName(selectedCell.col)}
          {selectedCell.row + 1}
          {editingCell && ' (Editing)'}
        </Typography>
