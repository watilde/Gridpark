...
import { CellValue, CellType, CellData, StoredCellData } from './db';
import { ExcelCellStyle, ExcelSheetProperties } from './exceljs-types';
...
export class ExcelJSAdapter {
   ...
   */
  static async readWorkbookMetadata(buffer: ArrayBuffer): Promise<{
    sheets: Array<{
      name: string;
      rowCount: number;
      colCount: number;
    ...
    const sheets = workbook.worksheets.map(worksheet => {
      ...
      const properties: ExcelSheetProperties = {
        ...
        autoFilter: worksheet.autoFilter
          ? {
              ref:
                typeof worksheet.autoFilter === 'string'
   ...
   */
  static async readWorkbook(buffer: ArrayBuffer): Promise<{
    sheets: Array<{
      name: string;
      data: CellData[][];
      properties?: ExcelSheetProperties;
    ...
    for (const [sheetIndex, sheetData] of sheets.entries()) {
      ...
      sheetData.data.forEach((row, rowIndex) => {
        row.forEach((cellData, colIndex) => {
          // Skip empty cells
          if (!cellData || (cellData.value === null && !cellData.formula && !cellData.style)) {
            return;
          }
      ...
      // Apply row heights
      if (sheetData.properties?.rows) {
        sheetData.properties.rows.forEach((rowDef, idx) => {
          if (!rowDef) return;
          const row = worksheet.getRow(idx + 1);
          if (rowDef.height !== undefined) row.height = rowDef.height;
          if (rowDef.hidden !== undefined) row.hidden = rowDef.hidden;
          if (rowDef.outlineLevel !== undefined) row.outlineLevel = rowDef.outlineLevel;
        });
      }

      // Apply merges (A1-style ranges, e.g. "A1:B2")
      if (sheetData.properties?.merges) {
        sheetData.properties.merges.forEach(range => {
          try {
            worksheet.mergeCells(range);
          } catch (e) {
            console.warn('[ExcelJS] Failed to merge range:', range, e);
          }
        });
      }
